...
const AddUser = () => {
  ...
  const onSubmitHandle = async (e) => {
    ...
    await axios.post("https://react-crud-backend-v1.herokuapp.com/find",{"email":email},{
            headers: {
    ...
    await axios.post('https://react-crud-backend-v1.herokuapp.com/add',user, {
        headers: {
        'Content-Type': 'application/json'
        }
      }
    ...
    <div className="container py-3 border mt-5 ">
        ...
        <div>
          <Modal show={showErrorEmail}>
            ...
            <Modal.Body>
              Please enter a different mail address , the above provided mail already exist
            </Modal.Body>
            <Modal.Footer>
                <Link className="btn btn-primary" onClick={()=>{setShowErrorEmail(false)}}>Ok</Link>
            </Modal.Footer>
          </Modal>
        </div>
      <h3 className="text-center mb-3 font-weight-light text-uppercase">Add A Student</h3>
